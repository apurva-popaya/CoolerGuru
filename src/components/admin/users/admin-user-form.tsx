...
import { Building2, Info, Store, UserRound } from "lucide-react";
...
import type { UserRole } from "./users-data";
...
export function AdminUserForm({
  ...
  const [mobile, setMobile] = React.useState(initialMobile.replace(/\D/g, "").slice(-10));
  ...
  const formIsValid = name.trim().length > 0 && mobile.length === 10;
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    const role = selectedRole === "Buyer" ? "BUYER" : "SELLER";
    ...
    try {
      if (mode === "edit") {
         ...
         * additional fields for POST /users.
         *
         * This call is intentionally wired
         * according to the final frontend
         * contract and may return 422 until
         * backend validation is updated.
         */
  ...
  return (
    <form onSubmit={handleSubmit} className="rounded-[10px] border border-border bg-white p-6">
      <div>
        ...
        <p className="mt-1 text-[#5d6280] text-[12px]">Select whether this account belongs to a buyer or supplier.</p>
      </div>
        ...
        {roleOptions.map((option) => {
          ...
          return (
            <button
              ...
              className={`relative min-h-[135px] rounded-[9px] border p-5 text-left transition ${selected ? "border-[#2720a8] bg-[#f5f4ff] shadow-[0_0_0_1px_#2720a8]" : "border-border bg-white hover:border-[#c9c6f3]"}`}
            >
        ...
        <div className="mt-6 space-y-5">
          <div>
            <label htmlFor="admin-user-name" className="font-semibold text-[#15136f] text-[13px]">
              {selectedRole === "Buyer" ? "Name" : "Company Name"} <span className="text-red-500">*</span>
            </label>
            ...
            <div className="relative mt-2">
              ...
              <Input
                ...
                placeholder={selectedRole === "Buyer" ? "Enter buyer name" : "Enter company name"}
      ...
      {error && (
        <div className="mt-5 rounded-[8px] border border-red-200 bg-red-50 p-3 text-[12px] text-red-600">{error}</div>
      )}
      ...
      <div className="mt-8 flex items-center justify-between border-border border-t pt-5">
        ...
        >
          {loading ? "Saving..." : mode === "edit" ? "Save Changes" : `Add ${selectedRole}`}
        </Button>
      </div>
    </form>
  );
