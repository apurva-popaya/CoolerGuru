...
export function ProductSummaryGrid({ products, status }: ProductSummaryGridProps) {
  ...
  return (
    <section className="relative mt-5 overflow-hidden rounded-[10px] border border-[#e1e2ed] bg-white">
      ...
      <div className={`grid grid-cols-4 gap-4 p-4 ${unlocked ? "" : "pointer-events-none select-none opacity-[0.25]"}`}>
        {products.map((product) => (
          <article key={product.id} className="rounded-[8px] border border-[#e2e3ed] bg-white p-3">
            <div className="relative h-[125px] w-full">
              <Image src={product.image} alt={product.title} fill sizes="250px" className="object-contain" />
            </div>

            <h3 className="mt-2 font-semibold text-[#171570] text-[11px]">{product.title}</h3>

            <div className="mt-2 flex items-center justify-between">
              <span className="text-[#70758e] text-[10px]">{unlocked ? product.units : "— Units"}</span>
