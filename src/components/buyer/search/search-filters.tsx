...
interface SearchFiltersProps {
  ...
  // Used only when the filter component is inside
...
export function SearchFilters({
  ...
  const [localBusinessTypes, setLocalBusinessTypes] =
    useState<SearchBusinessType[]>(businessTypes);

  const [localCategorySlugs, setLocalCategorySlugs] =
    useState<string[]>(categorySlugs);

  const [localVerifiedOnly, setLocalVerifiedOnly] =
    useState(verifiedOnly);

  const [localPremiumOnly, setLocalPremiumOnly] =
    useState(premiumOnly);
  ...
  const categoryOptions = facets?.categories?.length
    ? facets.categories
    : fallbackCategories;
  ...
  return (
    <aside
      className={
        isMobileDrawer
          ? "w-full bg-white"
          : "w-full rounded-[8px] border border-[#dedff0] bg-white p-4"
      }
    >
      {/* =========================================================
          FILTER HEADER
      ...
      <div
        className={`flex items-center justify-between ${
          isMobileDrawer
            ? "sticky top-0 z-10 border-[#e4e5ef] border-b bg-white px-5 py-4"
            : "mb-4"
        }`}
      >
        <h2 className="font-bold text-[#15166f] text-[15px]">
          Filters
        </h2>
        ...
        <div className="flex items-center gap-4">
          ...
          {/* Close button appears only inside mobile/tablet drawer */}
          {isMobileDrawer && onClose ? (
            <button
              type="button"
              onClick={onClose}
              aria-label="Close filters"
              className="flex h-8 w-8 items-center justify-center rounded-full border border-[#dedff0] text-[#2118ad] transition hover:bg-[#f4f3ff]"
            >
              <X size={17} />
            </button>
          ) : null}
      ...
      >
        ...
        <FilterSection title="Verified Company">
          <CheckboxRow
            label="Verified Only"
            count={String(
              facets?.verified_count ?? 325,
            )}
            checked={localVerifiedOnly}
            onChange={() =>
              setLocalVerifiedOnly(
                (current) => !current,
              )
            }
          />
        ...
        <FilterSection title="Location">
          <p className="mb-1.5 font-semibold text-[#313661] text-[9px]">
            State
          </p>
          ...
          <div className="space-y-2">
            {categoryOptions.map((item) => (
              <CheckboxRow
                ...
                label={item.name}
                count={String(item.count)}
                checked={localCategorySlugs.includes(
                  item.slug,
                )}
                onChange={() =>
  ...
  return (
    <div className="border-[#ececf4] border-b py-3 first:pt-0 last:border-b-0">
      <div className="mb-3 flex items-center justify-between">
        <h3 className="font-bold text-[#1a1b6f] text-[10px]">
          {title}
        </h3>

        <ChevronDown
          size={13}
          className="text-[#252681]"
        />
      </div>
...
function CheckboxRow({
  ...
  return (
    <label className="flex cursor-pointer items-center gap-2">
      ...
      <span className="min-w-0 flex-1 text-[#33375f] text-[9px]">
        {label}
      </span>

      <span className="font-semibold text-[#313179] text-[9px]">
        {count}
      </span>
    </label>
  );
