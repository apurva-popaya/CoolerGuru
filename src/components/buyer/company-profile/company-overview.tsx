...
export function CompanyOverview({
  ...
  const hasAbout = Boolean(company.about?.trim());
  const hasCategories = company.productCategories.length > 0;

  // Don't render the entire section if there is no overview data.
  if (!hasAbout && !hasCategories) {
    return null;
  }
    ...
    <div className="mt-4 rounded-[8px] border border-[#e2e3ee] bg-white p-3 sm:p-4">
      ...
      <div
        className={`mt-3 grid grid-cols-1 gap-5 ${
          hasAbout && hasCategories
            ? "md:grid-cols-[1fr_1px_1fr] md:gap-5"
            : ""
        }`}
      >
        {/* About */}
