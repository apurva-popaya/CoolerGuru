...
export function BuyerInquiriesPagination({ pagination, onPageChange, onLimitChange }: Props) {
  const start = pagination.totalItems === 0 ? 0 : (pagination.page - 1) * pagination.limit + 1;

  const end = Math.min(pagination.page * pagination.limit, pagination.totalItems);

  return (
    <div className="grid grid-cols-[1fr_auto_1fr] items-center border-[#ececf2] border-t px-4 py-3">
      <p className="text-[#555a75] text-[8.5px]">
