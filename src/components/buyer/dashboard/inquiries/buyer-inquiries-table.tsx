...
export function BuyerInquiriesTable({ inquiries, loading = false }: Props) {
  return (
    <div>
      ...
      {loading ? (
        <div className="px-4 py-10 text-center text-[#686d84] text-[9px]">Loading inquiries...</div>
      ) : inquiries.length === 0 ? (
        <div className="px-4 py-10 text-center text-[#686d84] text-[9px]">No inquiries found.</div>
      ) : (
        inquiries.map((inquiry) => (
          ...
          >
            <p className="font-medium text-[#30355b] text-[8.5px]">{inquiry.inquiry_number}</p>

            <div className="min-w-0">
              <p className="truncate font-bold text-[#171570] text-[9px]">{inquiry.company?.name ?? "Supplier"}</p>
            ...
            <p className="truncate pr-4 text-[#454a67] text-[8.5px]">{inquiry.product_requirement}</p>

            <p className="text-[#454a67] text-[8.5px]">
              {inquiry.quantity} {inquiry.quantity_unit}
            </p>

            <p className="text-[#454a67] text-[8.5px]">{formatDate(inquiry.created_at)}</p>
