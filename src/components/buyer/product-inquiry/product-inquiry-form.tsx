...
export function ProductInquiryForm({ product }: ProductInquiryFormProps) {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
       ...
       *
       * This currently assumes product.id
       * contains a slug such as:
       *
       * industrial-air-cooler-ic-15000-test
       *
       * Once ProductDetail has an explicit
       * slug field, change this to:
       *
       * product.slug
       */
      const response = await createProductInquiry(product.id, {
      ...
      setSuccess(response.message || "Product inquiry submitted successfully.");

      /*
       * Keep buyer contact information
       * because the user may send another
       * inquiry, but clear inquiry-specific
       * fields.
       */
      setQuantity("");
  ...
  return (
    <form onSubmit={handleSubmit} className="rounded-[10px] border border-[#e1e2ed] bg-white p-6">
      <h2 className="font-bold text-[#171570] text-[18px]">Product Inquiry Form</h2>
        ...
        <FormField label="Quantity">
          <div className="grid grid-cols-[1fr_105px] gap-3">
            ...
            <select
              ...
              className="h-[44px] rounded-[5px] border border-[#dedff0] bg-white px-3 text-[#454a69] text-[10px] outline-none"
              disabled={loading}
            >
      ...
      <div className="mt-5 grid grid-cols-2 gap-5">
        <FormField label="Mobile Number">
          <div className="flex h-[44px] overflow-hidden rounded-[5px] border border-[#dedff0] bg-white">
            <button
              type="button"
              className="flex w-[84px] shrink-0 items-center justify-center gap-2 border-[#dedff0] border-r font-medium text-[#34395e] text-[10px]"
              disabled={loading}
            >
              +91
              <ChevronDown size={11} />
        ...
        <FormField label="Requirement Details">
          <div className="relative">
            <textarea
              ...
              className="min-h-[105px] w-full resize-none rounded-[5px] border border-[#dedff0] bg-white px-3 py-3 pb-7 text-[#292e50] text-[11px] leading-[1.5] outline-none transition placeholder:text-[#a4a7b8] focus:border-[#8c83e8]"
              disabled={loading}
              required
            />
      ...
      <div className="mt-5 flex items-center gap-4">
        <button
          type="submit"
          disabled={loading}
          className="flex h-[40px] min-w-[255px] items-center justify-center gap-2 rounded-[5px] bg-[#2116a5] px-6 font-bold text-[10px] text-white transition hover:bg-[#3022c6] disabled:cursor-not-allowed disabled:opacity-60"
        >
          <Send size={14} />
        ...
        <Link
          ...
          className="!text-[#251bb4] flex h-[40px] min-w-[210px] items-center justify-center rounded-[5px] border border-[#3929dd] bg-white px-5 font-bold text-[10px] transition hover:bg-[#f6f5ff]"
        >
          Cancel
        </Link>
      </div>
      ...
      <div className="mt-3 flex items-center gap-2">
        <ShieldCheck size={13} className="text-[#3025ce]" />

        <p className="text-[#696e84] text-[8px]">
          The supplier will receive your inquiry by email and dashboard notification.
        </p>
      </div>
    </form>
  );
...
function FormField({ label, children }: FormFieldProps) {
  return (
    <div>
      <label className="mb-2 block font-bold text-[#171570] text-[10px]">{label}</label>
