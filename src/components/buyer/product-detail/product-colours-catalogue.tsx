...
export function ProductColoursCatalogue({ product }: { product: ProductDetail }) {
  return (
    <div className="rounded-[8px] border border-[#e2e3ee] bg-white p-4">
      ...
      <div className="mt-3 flex items-start gap-5">
        {product.colours.map((colour) => (
          <div key={colour.name} className="text-center">
            <div className={`h-[24px] w-[24px] rounded-full border border-[#353a60] ${colour.className}`} />
      ...
      <h2 className="mt-4 border-[#ececf3] border-t pt-3 font-bold text-[#171570] text-[11px]">Video & Catalogue</h2>

      <div className="mt-2 grid grid-cols-[1.1fr_0.9fr] gap-2">
        <Link
          href={product.videoUrl}
          className="relative h-[70px] overflow-hidden rounded-[5px] border border-[#e2e3ee]"
        >
          <Image
        ...
        <div className="flex items-center gap-2 rounded-[5px] border border-[#e2e3ee] p-2">
          <FileText size={24} className="shrink-0 text-[#4334dc]" />

          <div className="min-w-0 flex-1">
            <p className="truncate font-bold text-[#262174] text-[7px]">{product.catalogueTitle}</p>

            <p className="mt-0.5 text-[#777b90] text-[6px]">{product.catalogueSize}</p>
